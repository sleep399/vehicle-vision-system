...
class OwnerGestureService:
    ...
    def _classify_static_gesture(self, landmarks, w: int, h: int) -> Tuple[str, float]:
        ...
        if ext["index"] and non_thumb_count <= 2 and not ext["ring"] and not ext["pinky"]:
            ...
            palm_ref = self._distance((landmarks[HandLandmark.INDEX_FINGER_MCP].x,
                                       landmarks[HandLandmark.INDEX_FINGER_MCP].y),
                                      (landmarks[HandLandmark.PINKY_MCP].x,
                                       landmarks[HandLandmark.PINKY_MCP].y))
            if palm_ref > 1e-3 and finger_len > palm_ref * 0.30 and abs(dx) > abs(dy) * 0.65:
                return ("point_right", 0.86) if dx > 0 else ("point_left", 0.86)
            return "point", 0.75
    ...
    def _detect_motion_gesture(self, landmarks, w: int, h: int, static_gesture: str) -> Tuple[str, float]:
        ...
        if (static_gesture in ("point", "point_left", "point_right")
            ...
            return ("point_right", confidence) if swipe_dx > 0 else ("point_left", confidence)
    ...
    def _apply_realtime_confirmation(
        ...
        if gesture == "no_gesture":
            if (
                self._realtime_candidate_gesture != "no_gesture"
                and now - self._realtime_candidate_since <= self.REALTIME_KEEP_SEC
            ):
                debug["hold"] = "keep_candidate_gap"
                return "no_gesture", 0.0, debug
            if (
                self._realtime_confirmed_gesture != "no_gesture"
                and now - self._realtime_confirmed_at <= self.REALTIME_KEEP_SEC
            ):
    ...
    def recognize(
        ...
        if detected_hands:
            ...
            if (
                realtime_mode
                and static_g in ("palm_open", "point", "point_left", "point_right")
    ...
    def _annotate(self, annotated, en, confidence, action, needs_confirmation):
        ...
        cv2.putText(annotated, f"{en} ({confidence:.0%})", (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
        if needs_confirmation:
            cv2.putText(annotated, "Low confidence - confirm?", (20, 75),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        elif action:
            cv2.putText(annotated, f"Action: {action}", (20, 75),
